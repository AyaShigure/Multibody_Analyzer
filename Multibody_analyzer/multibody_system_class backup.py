...
class system_full_nonlinear_dynamics():
    def __init__(self, 
        ...
        #↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓ Lambdify() ↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓
        self.system_Mass_matrix_lambda
        ...
        print(bcolors.BOLD + bcolors.FAIL +'Multibody system is created.' + bcolors.ENDC)

    # Pass in: 
    #   t: Sympy symbol, t
    #   g: Sympy symbol, g
    #   BSP_alpha: Baumgarte's Stabilization Parameter alpha, (Pass in 0 if there is no constraint condition)
    def calculate_StateSpace_Lambdify_ConstraintForce_OdeForSolver(self, t, g, BSP_alpha):
        ...
        # # q_dot term lambdify()
        # q_dot_lambda = []
        # for item in self.system_generalized_velocities:
        #     q_dot_lambda.append(lambdify(symbols_for_lambdify, item))
        ...
        def ODE(t, X, pbar, state, g, *Control_force_vector):
            ...
            # Calculate q_dot and q_double dot term here


            # constraint_force_in_q_coord = clear_list_to_0(constraint_force_in_q_coord)
            constraint_force_in_q_coord = []
            for i in range(len(self.system_generalized_coordinates)):
                constraint_force_in_q_coord.append(0)
            ...
            #     # Calculate i-th constraint force and sum them together in the end
            #     for i in range(len(self.system_constraint_equation_objects)):
            #         h_Vec = constraint_equation_lambda[i](t, g, *state_vector_dummy, *Control_force_vector)
            #         H_Mat = constraint_jacobian_lambda[i](t, g, *state_vector_dummy, *Control_force_vector)
            #         H_dot_Mat = constraint_jacobian_time_derivative_lambda[i](t, g, *state_vector_dummy, *Control_force_vector)

            #         constraint_force_in_xyz = -fast_pinv(H_Mat * Mass_Matrix_inv *np.transpose(H_Mat)) * (H_Mat * Mass_Matrix_inv * (Gravtational_Force_Vector + Control_Matrix * Control_vector - Coriolis_Vector) + (2*BSP_alpha*H_Mat + H_dot_Mat)*q_dot_vec_current + BSP_alpha**2 * h_Vec)
            ...
            # print('qddt')
            # print(q_double_dot )
            # state_space_model_RHS is [q_dot, q_double_dot]
            # print(q_dot_lambda(t, g, *state_vector_dummy, *Control_force_vector))
            ...
            # symbols_for_lambdify = ((t) , (g) , *tuple(self.system_state_vector) , *tuple(self.system_Control_force_vector))
            # print('###################################################')
            # print(nonlinear_state_space_model_RHS)
            # Progress bar
            last_t, dt = state
            n = int((t - last_t)/dt)
            pbar.update(n)
